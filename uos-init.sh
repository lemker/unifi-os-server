...
# Import container environment variables set via Dockerfile ENV or -e
for var in APP_MODEL APP_VERSION PRODUCT_NAME UOS_UUID UOS_SYSTEM_IP HARDWARE_PLATFORM; do
    val=$(grep -z "^${var}=" /proc/1/environ 2>/dev/null | tr -d '\0' | cut -d= -f2-)
    if [ -n "$val" ]; then
        export "$var=$val"
    fi
done
